...
def door_return(image):
    ...
    for a in cont:
       ...
       for i in range(len(locs)):
           ...
           if (x<=x1<=(x+w)) and (y<=y1<=(y+h)) and ((w/h)<0.8) and (2<rat<5) and (h>(2*w)) and (h>100) and (w>100):
               counter+=1
...
while(True):
    ...
    for index in range(len(doors)):
        ...
        if knobs[index]:
            x2,y2 = knobs[index]
            cv2.circle(frame,(x2,y2),13,127,thickness=4, lineType=8, shift=0)
        '''
